...
export default function DashboardModal({modalRef, closeModal, isSuccess, error, info }: {modalRef: undefined | LegacyRef<HTMLDialogElement>, closeModal: ()=>void,   isSuccess: boolean, error:any, info:string}) {

  
  return (
    
      <dialog ref={modalRef} className="animate-in zoom-in duration-100  ease-in-out  w-2/3 md:w-1/4  rounded-2xl h-52  p-4  backdrop:bg-secondary-dark backdrop:opacity-80 dark:bg-primary-dark">
    ...
    <div className = "flex items-center justify-center">
        ...
        <div className="flex flex-col items-center justify-center w-full">
          ...
          <h2 className="text-center leading-5 text-md mb-2 animate-in slide-in-from-bottom duration-100">{info}!</h2>
          ...
          <Button variant={'default'} className="animate-in slide-in-from-bottom delay-75 w-1/2 " onClick={closeModal}> Close </Button>
          </div>
        </div>
        </div>
        </div>
